...
public class SessionThread extends SocketAction{
    ...
    public void run() {
        ...
        while(!checkPointsofAllPLayers()){
                ...
                for(int i = 1; i <= playerThreadsWithPlayerNumbers.size(); i++){
                    try {
                        while (true) {
                            ...
                            if (Pattern.matches("[0-9]+", tempo)&&playersWithHands.get(i).contains(Integer.parseInt(tempo))) {
            ...
            playerThreadsWithPlayerNumbers.get(firstBingo).send("you have all same cards in your hand.Write bingo to win: ");
            try {
            ...
            for (int i=1;i<=playerThreadsWithPlayerNumbers.size();i++) {
                if(i != firstBingo)
                playerThreadsWithPlayerNumbers.get(i).send("player "+firstBingo+" said bingo. Write bingo  to get points");
            }
